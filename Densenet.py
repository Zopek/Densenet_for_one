...
class DenseNet():
    def __init__(self, x, voxels, nb_blocks, filters, training):
        self.voxels = voxels
    ...
    def Dense_net(self, input_x):
        ...
        x = Relu(x)
        x = Global_Average_Pooling(x, voxel=self.voxels)
        x = flatten(x)
        x = Linear(x)
...
# 维数的占位符
voxel = tf.placeholder(tf.float32)
...
logits = DenseNet(x=batch_images, voxels=voxel, nb_blocks=nb_block, filters=growth_k, training=training_flag).model
...
with tf.Session() as sess:
    ...
    for epoch in range(total_epochs):
        ...
        for step in range(total_train_batch):
            ...
            batch_x, batch_voxel, batch_y = input_data.train_next_batch(filepath, step, batch_size)
        ...
        for step in range(total_test_batch):
            test_images, test_voxel, test_labels = input_data.test_next_batch(filepath, step)
            test_feed_dict = {
                x: test_images,
                voxel: test_voxel,
                label: test_labels,
